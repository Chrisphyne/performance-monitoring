              ...
              <tr key={`entity-${i}`} onClick={() => props.history.push(`/devices/${/[^/]*$/.exec(device.name)[0]}`)}>
                ...
                <td>
                  ...
                  <div className="small text-muted">
                    <span>{device.ownership}</span> | {device.lastStatusReportTime}
                  </div>
                </td>
                <td>
                ...
                <td>
                  <div className="clearfix">
                    ...
                    <div className="float-right">
                      <small className="text-muted">{getLastBatteryLevelCollectedEvent(device) ? getLastBatteryLevelCollectedEvent(device).createTime : "Current Policy might not have power events enabled"}</small>
                    </div>
                  </div>
                  {
                    device.powerManagementEvents && device.powerManagementEvents.length > 0
                    && device.powerManagementEvents[device.powerManagementEvents.length - 1].batteryLevel > 50
                    && <CProgress className="progress-xs" color="success" value={device.powerManagementEvents[device.powerManagementEvents.length - 1].batteryLevel} />
                  }
                  {
                    device.powerManagementEvents && device.powerManagementEvents.length > 0
                    && (device.powerManagementEvents[device.powerManagementEvents.length - 1].batteryLevel <= 50 && device.powerManagementEvents[device.powerManagementEvents.length - 1].batteryLevel >= 15)
                    && <CProgress className="progress-xs" color="warning" value={device.powerManagementEvents[device.powerManagementEvents.length - 1].batteryLevel} />
                  }
                  {
                    device.powerManagementEvents && device.powerManagementEvents.length > 0
                    && device.powerManagementEvents[device.powerManagementEvents.length - 1].batteryLevel < 15
                    && <CProgress className="progress-xs" color="danger" value={device.powerManagementEvents[device.powerManagementEvents.length - 1].batteryLevel} />
                  }
                </td>
                <td>
                  <div className="clearfix">
                    <div className="float-left">
                      <strong>{device.memoryEvents && device.memoryEvents.length > 0 ? (parseInt(getLastExternalStorageMeasuredEvent(device).byteCount, 10) / (1024*1024*1024)).toFixed(2)+' GB Available': ""}</strong>
                    </div>
                    <div className="float-right">
                      <small className="text-muted">{device.memoryEvents && device.memoryEvents.length > 0 ? (parseInt(getLastExternalStorageDetectedEvent(device).byteCount, 10) / (1024*1024*1024)).toFixed(2)+' GB Total' : "Applied Policy doesn't have memory events"}</small>
                    </div>
                  </div>
                  {
